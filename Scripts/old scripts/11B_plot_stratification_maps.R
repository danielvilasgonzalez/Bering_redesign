...
r3<-r3[complete.cases(r3$layer),] 
r3$layer<-as.character(r3$layer)
...
plot_list[['baseline']]<-p


cowplot::plot_grid(plot_list[['baseline']],plot_list[[3]],plot_list[[2]],plot_list[[1]])
  ...
  ragg::agg_png(paste0('./figures/sampling designs.png'), width = 10, height = 10, units = "in", res = 300)
  cowplot::plot_grid(plot_list[['baseline']],plot_list[[3]],plot_list[[2]],plot_list[[1]])
  dev.off()
