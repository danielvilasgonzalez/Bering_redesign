...
load(file = paste0("./output/species/historical design estimates.RData" )) #index_hist
...
for (sp in spp) {
  #sp<-spp[1]
  true_ind[,sp]<-drop_units(dens_index_hist_OM[[sp]]$index[,,1])
}
...
#p<-
  ...
  #facet_wrap(~com_sci,scales='free',dir='v',nrow = 5)
...
rrmse<-array(NA,
             dim=list(length(spp),length(yrs),2,5),
