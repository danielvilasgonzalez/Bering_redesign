...
#load(paste0('./output/multisp_optimization_static_data.RData')) #temp_dens_vals
D6<-df
...
#samp_df<-samp_df[order(match(samp_df$samp_scn,c('scnbase','scnbase_bis','scn5','scn6','scn7','scn1','scn2','scn3','scn9','scn10','scn11','scn12'))),]
samp_df<-subset(samp_df,samp_scn %in% c('scn3','scn2','scn1','scn8','scn7','scn6','scn5','scn11','scn10','scn9','scn15','scn14','scn13'))
samp_df<-samp_df[order(match(samp_df$samp_scn,c('scn3','scn2','scn1','scn8','scn7','scn6','scn5','scn11','scn10','scn9','scn15','scn14','scn13'))),]
scns<-unique(samp_df$samp_scn)
samp_df$n<-1:nrow(samp_df)
#samp_df<-samp_df[1:12,]
...
for (samp in samp_df$samp_scn) {
  ...
  domain<-samp_df[which(samp_df$samp_scn==samp),'domain']
  ...
  # sys<-rbind(sys1,as.data.frame(syst))[,c('Lon',"Lat")]
  ...
  # sb<-data.frame(sb)
    ...
    load(file = paste0("./output/ms_optim_allocations_ebsnbs_slope_",samp,'_DOM',domain,".RData")) #all
    
    allsum<-all$result_list$solution$framenew[,c('ID','DOMAINVALUE','STRATO','LABEL')]
    names(allsum)<-c('cell','domain','Strata','label')
    D8<-merge(D6,allsum,by='cell')
    D8$Strata2<-paste0(D8$domain,D8$Strata)
    ...
    #n cells by strata
    strata_sum<-aggregate(cell ~ Strata + domain, data = allsum, FUN = length)
    names(strata_sum)<-c('Strata','domain','total_cell')
    strata_sum<-cbind(strata_sum,'n_samples'=all$samples_strata$n_samples)
    #merge
    #strata<-merge(strata,all$samples_strata,by.x='Strata',by.y='strata')
    strata<-merge(D8,strata_sum,by=c('Strata','domain'))
  ...
  D8$Strata2<-paste0(D8$Strata,D8$domain)
  D8$Strata2<-as.integer(D8$Strata2)
  ...
  #create raster
  r2<-rasterize(D8_1, r1 ,field=c('Strata','n_samples','prop','domain','Strata2'))
  ...
  r4<-rasterToPolygons(r2$Strata2,dissolve=TRUE,digits = 1)
  #plot(r4)
  ...
  # # Join the shapefiles based on a common attribute
  # joined_shapefile <- merge(EBSshelf_sh, NBS_sh, by="OBJECTID")
  # plot(joined_shapefile)
  # # Dissolve boundaries based on a grouping variable
  # dissolved_shapefile <- unionSpatialPolygons(bs_sh1, bs_sh1$OBJECTID)
  # plot(dissolved_shapefile)
  ...
  ggplot()+
    ...
    geom_polygon(data=EBSslope_sh,aes(x=long,y=lat,group=group), colour=rgb(0, 0, 0, 0.4),alpha=0.2, fill=NA)+
    ...
    coord_sf(crs = '+proj=aea +lat_1=55 +lat_2=65 +lat_0=50 +lon_0=-154 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs',
             xlim = c(-1382205, -224348.5),
             ylim = c(537256.1, 1823031),
             label_axes = "-NE-")+
...
pp<-cowplot::plot_grid(plotlist = plot_list[c(1,2,3)],nrow = 1,ncol=3)+
  theme(panel.border = element_rect(fill = NA, colour = 'grey'))

#pp1<-cowplot::plot_grid(pp,legend2,nrow = 2,ncol=1,rel_heights = c(1,0.01))

#ragg::agg_png(paste0('./figures/stratification_mapbis15strata.png'), width = 15, height = 10, units = "in", res = 300)
print(pp)
#dev.off()

pp<-cowplot::plot_grid(plotlist = plot_list[c(4:7)],nrow = 1,ncol=4)+
  theme(panel.border = element_rect(fill = NA, colour = 'grey'))

#pp1<-cowplot::plot_grid(pp,legend2,nrow = 2,ncol=1,rel_heights = c(1,0.01))

#ragg::agg_png(paste0('./figures/stratification_mapbis10strata.png'), width = 15, height = 10, units = "in", res = 300)
print(pp)
#dev.off()

pp<-cowplot::plot_grid(plotlist = plot_list[c(8:10)],nrow = 1,ncol=3)+
  theme(panel.border = element_rect(fill = NA, colour = 'grey'))

#pp1<-cowplot::plot_grid(pp,legend2,nrow = 2,ncol=1,rel_heights = c(1,0.01))

#ragg::agg_png(paste0('./figures/stratification_mapbis5strata.png'), width = 15, height = 10, units = "in", res = 300)
print(pp)
#dev.off()


pp<-cowplot::plot_grid(plotlist = plot_list[c(11:13)],nrow = 1,ncol=3)+
  theme(panel.border = element_rect(fill = NA, colour = 'grey'))
