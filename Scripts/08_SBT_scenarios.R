...
pack_cran<-c('ncdf4','raster','FNN','lubridate','rasterVis','rgeos','scales','rnaturalearth','grid','ggplot2')
...
r1 <- raster(ext=extent(df2022),res=c(15800,15800))
...
for (scn in unique(df_scn$scn_n)) {
  ...
  #save raster
  writeRaster(sbt_stack, paste0('data processed/SBT scenarios/SBT_scn',scn,'_',paste0(range(yrs)[1],'-',range(yrs)[2]),'.grd'))
  
  #save plot
  png(paste0('./figures/sbt_scenario_',scn,'.png'), width = 700, height = 190)
  print(
    levelplot(sbt_stack,
              layout=c(6, 1),
              col.regions=pal,
